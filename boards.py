...
class CharucoBoard(CalibrationObject):
    ...
    def detect_markers(self, image, camera=None, refine=True):
        ...
        params.cornerRefinementMethod = aruco.CORNER_REFINE_CONTOUR # 3.4.19
        ...
        rejectedImgPoints_x = []
        rejectedImgPoints_y = []
        for i in range(len(rejectedImgPoints)):  # 62 "groups"
            for j in range(4):  # 4 rows in each group
                for k in range(2):  # 2 cols in each group
                    for l in range(2):
                        for m in range(2):
                            if m == 0:
                                rejectedImgPoints_x.append(rejectedImgPoints[m][l][k][j][i])
                            if m == 1:
                                rejectedImgPoints_y.append(rejectedImgPoints[m][l][k][j][i])
